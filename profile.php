<?php 
    ...
    mysqli_select_db($dbc, 'cooking_corner');
     ...
     if (isset($_POST['submit'])){
            ...
            //Define the query
            $query = "UPDATE user SET pword='{$_POST['pWord']}' 
            ...
            if (mysqli_affected_rows($dbc) ==1){
                    header ('location: profile.php?msg= Your password has changed successfully!');
                    exit();
            ...
            //Define the query
            $query = "UPDATE user SET email='{$_POST['NeweMail']}' 
                      WHERE user_id = '{$_SESSION['user_id']}'";
            ...
            //Report on the result
            if (mysqli_affected_rows($dbc) ==1){
                    header ('location: profile.php?msg2= Your email has changed successfully!');
                    exit();
        ...
        // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["profile"]["tmp_name"], $target_file)) {
                 $query = "UPDATE user SET profile='$profile' WHERE user_id= '{$_SESSION['user_id']}'";

                if (mysqli_query($dbc, $query)) {
                    $_SESSION['profile'] = $profile;
                    echo '<script>alert("Your Profile for ' . htmlspecialchars(basename($_FILES["profile"]["name"])) . ' has been uploaded.")</script>';
                    $value = true;
                }
            } else {
...
<head>
    ...
    <!-- Title -->
    <title>The Cooking Corner | My Profile</title>
</head>
   ...
   <?php include'components/nav-bar-login.php';?>
    ...
    if(isset($_GET['msg'])) {
        echo '<div class="bg-success" style="border-radius: 5px; padding:6px 2px 6px 2px; margin: 0px 50px 0px 50px;">';
            echo '<h7 class="p-1 pl-3" style="color:#FFF;">'. $_GET['msg'] .' <a href="profile.php" class="float-right pr-3" style="color:white;">Close</a></h7>';
        echo '</div>';
    }elseif (isset($_GET['msg2'])) {
        echo '<div class="bg-success" style="border-radius: 5px; padding:6px 2px 6px 2px; margin: 0px 50px 0px 50px;">';
            echo '<h7 class="p-1 pl-3" style="color:#FFF;">'. $_GET['msg2'] .' <a href="profile.php" class="float-right pr-3" style="color:white;">Close</a></h7>';
        echo '</div>';
    }
    ?>
   <br/>
    <div class="container">
        <div class="row">
              <div class="col-md-3">
                ...
                <?php 
                    print '
                    <a href="deleteAccount.php?user_id='. $_SESSION['user_id'] .'" 
    ...
    <form name="profilePasswordForm" action="profile.php" onsubmit="return validatePasswordForm()" method="post">
    ...
    <form name="profileEmailForm" action="profile.php" onsubmit="return validateEmailForm()" method="post">
...
<form name="profilePictureForm" action="profile.php" onsubmit="return validateProfileForm()" method="post" enctype="multipart/form-data">
    ...
    function validateProfileForm(){
         var e = document.forms["profilePictureForm"]["profile"].value;
