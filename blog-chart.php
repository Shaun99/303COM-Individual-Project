<?php 
 ...
 if($conn = mysqli_connect('localhost', 'root', '', 'cooking_corner')) {
    ...
    $query = "SELECT dish, count(*) as number FROM recipe_blog GROUP BY dish";  
    $result = mysqli_query($conn, $query); 
    
    $query2 = "SELECT cat, count(*) as number FROM tips_blog GROUP BY cat";  
    $result2 = mysqli_query($conn, $query2); 
    
    $query3 = "SELECT rating, count(*) as number FROM blog_review GROUP BY rating";  
...
<body>
   <?php include'components/nav-bar-admin-login.php';?>
    ...
    {  
         ...
         var options = {  
               title: 'Blog Tips & Advices Categories',  
               //is3D:true,  
               pieHole: 0.4  
              };  
